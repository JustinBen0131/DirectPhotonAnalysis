...
v#include "AnalyzeTriggerGroupings.h"
#include "sPhenixStyle.h"
#include "sPhenixStyle.C"
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for (const auto& jetTrig : jetTriggersInCombination)
                        {
                            ...
                            // Attempt fit if enabled
                            if (enableFits)
                            {
                                // Retrieve known fit parameters
                                std::pair<std::string, std::string> key = std::make_pair(combinationName, jetTrig);
                                auto it_fitParams = TriggerConfig::triggerFitParameters.find(key);

                                // Try fallback by stripping firmware, etc.
                                if (it_fitParams == TriggerConfig::triggerFitParameters.end()) {
                                    std::string comboNoFirmware = Utils::stripFirmwareTag(combinationName);
                                    key = std::make_pair(comboNoFirmware, jetTrig);
                                    it_fitParams = TriggerConfig::triggerFitParameters.find(key);
                                }

                                // Final fallback with an empty combo
                                if (it_fitParams == TriggerConfig::triggerFitParameters.end()) {
                                    key = std::make_pair("", jetTrig);
                                    it_fitParams = TriggerConfig::triggerFitParameters.find(key);
                                }

                                if (it_fitParams != TriggerConfig::triggerFitParameters.end())
                                {
                                    // We have fit parameters
                                    const DataStructures::FitParameters& params = it_fitParams->second;

                                    // Give each fit a unique name (to avoid overwriting):
                                    std::string fitName = "fit_JetRatio_" + jetTrig;
                                    TF1* fitFunc = nullptr;

                                    // Sigmoid vs. erf
                                    if (fitFunctionType == "sigmoid") {
                                        fitFunc = Utils::sigmoidFit(
                                            fitName.c_str(),
                                            0.0, 50.0,
                                            params.amplitudeEstimate,
                                            params.slopeEstimate,
                                    ...
                                    if (fitFunc) {
                                        ...
                                        if (fitFunctionType == "sigmoid") {
                                            double k  = fitFunc->GetParameter(1);
                                            double x0 = fitFunc->GetParameter(2);
                                            x95 = x0 + std::log(95.0) / k;
                                        }
                                        else if (fitFunctionType == "erf") {
                                            double x0   = fitFunc->GetParameter(1);
                                            double sigma= fitFunc->GetParameter(2);
                                            double erfInvVal = TMath::ErfInverse(0.98);
                                            x95 = x0 + std::sqrt(2.) * sigma * erfInvVal;
                                        }

                                        // Store in map
                                        combinationToTriggerEfficiencyPoints[combinationName][jetTrig] = x95;

                                        // Optionally draw a vertical line at x95
                                        if (x95 > ratioJet->GetXaxis()->GetXmin() &&
                                            x95 < ratioJet->GetXaxis()->GetXmax())
                                        {
                                ...
                                {
                                    std::cout << "[INFO] No Jet fit parameters found for "
                                              << jetTrig << " in combination " << combinationName << "\n";
                                }
                            } // end if (enableFits)
