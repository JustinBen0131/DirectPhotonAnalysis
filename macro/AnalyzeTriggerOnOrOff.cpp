...
void summarizeTriggerCountsFromCSV(const std::string& csvFilePath, const std::vector<int>& triggerIndices) {
    ...
    for (int triggerIndex : triggerIndices) {
        std::cout << "Trigger " << triggerIndex << " (" << triggerNameMap[triggerIndex] << "):\n";
...
void AnalyzeTriggerOnOrOff() {
    ...
    summarizeTriggerCountsFromCSV(csvOutputFile, triggerBits);
}
