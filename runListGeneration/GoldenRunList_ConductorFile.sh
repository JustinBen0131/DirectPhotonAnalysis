...
# We do this for *both* DST_JET_run2pp and DST_CALO_run2pp.
generate_dst_lists() {
    if $DONT_GENERATE_FILELISTS; then
...
EOF
        ...
        # Use a pre-generated file by default
    ...
    while IFS= read -r line; do
        ...
        # 1) Remove from dst_jet_run2pp-<run>.list
        listfile_jet="${workplace}/../dst_list/dst_jet_run2pp-${run_part}.list"
        echo "[DEBUG] Looking for DST_JET list file: $listfile_jet"
        if [[ -f "$listfile_jet" ]]; then
            pattern="${run_part}-${seg_part}.root"
            echo "[DEBUG] Searching for pattern '$pattern' in $listfile_jet"
            lines_to_remove=$(grep "$pattern" "$listfile_jet")
            if [[ -n "$lines_to_remove" ]]; then
                echo "[INFO] Found these lines in $listfile_jet matching '$pattern':"
                echo "$lines_to_remove"
                grep -v "$pattern" "$listfile_jet" > "${listfile_jet}.tmp" && mv "${listfile_jet}.tmp" "$listfile_jet"
                echo "[INFO] Removed segment '$pattern' from $listfile_jet."
            else
                echo "[INFO] No matching lines found for segment '$pattern' in $listfile_jet. Skipping removal."
            fi
        else
            echo "[WARNING] DST_JET list file $listfile_jet does not exist. Skipping segment '$seg_part' for run '$run_part'."
        fi

        # 2) Remove from dst_calo_run2pp-<run>.list
        listfile_calo="${workplace}/../dst_list/dst_calo_run2pp-${run_part}.list"
        echo "[DEBUG] Looking for DST_CALO list file: $listfile_calo"
        if [[ -f "$listfile_calo" ]]; then
            pattern="${run_part}-${seg_part}.root"
            echo "[DEBUG] Searching for pattern '$pattern' in $listfile_calo"
            lines_to_remove=$(grep "$pattern" "$listfile_calo")
            if [[ -n "$lines_to_remove" ]]; then
                echo "[INFO] Found these lines in $listfile_calo matching '$pattern':"
                echo "$lines_to_remove"
                grep -v "$pattern" "$listfile_calo" > "${listfile_calo}.tmp" && mv "${listfile_calo}.tmp" "$listfile_calo"
                echo "[INFO] Removed segment '$pattern' from $listfile_calo."
            else
                echo "[INFO] No matching lines found for segment '$pattern' in $listfile_calo. Skipping removal."
            fi
        else
            echo "[WARNING] DST_CALO list file $listfile_calo does not exist. Skipping segment '$seg_part' for run '$run_part'."
        fi
    ...
    # ---------------------------------------------------------------------
    # Optionally: a quick check for segment consistency (JET vs. CALO).
    # ---------------------------------------------------------------------
    echo "[INFO] Performing a quick consistency check of JET/CALO segments..."
    local base_dir="${workplace}/../dst_list"
    for f in "${base_dir}/dst_jet_run2pp-"*.list; do
        [ -e "$f" ] || continue
        bn=$(basename "$f" .list)
        runnum_str=${bn#dst_jet_run2pp-}
        list_calo="${base_dir}/dst_calo_run2pp-${runnum_str}.list"

        if [[ ! -f "$list_calo" ]]; then
            echo "[WARNING] For run ${runnum_str}, no DST_CALO_run2pp .list found. Cannot compare segments."
            continue
        fi

        # Build sets of segment IDs
        declare -A seg_jet=()
        while IFS= read -r line_jet; do
            [[ -z "$line_jet" ]] && continue
            seg_part="${line_jet##*-}"
            seg_part="${seg_part%%.root}"
            seg_jet["$seg_part"]=1
        done < "$f"

        declare -A seg_calo=()
        while IFS= read -r line_calo; do
            [[ -z "$line_calo" ]] && continue
            seg_part="${line_calo##*-}"
            seg_part="${seg_part%%.root}"
            seg_calo["$seg_part"]=1
        done < "$list_calo"
        ...
        for s in "${!seg_jet[@]}"; do
            if [[ -z "${seg_calo[$s]}" ]]; then
                ((missing_in_calo++))
            fi
        done
        for s in "${!seg_calo[@]}"; do
            if [[ -z "${seg_jet[$s]}" ]]; then
                ((missing_in_jet++))
            fi
        done

        if (( missing_in_calo == 0 && missing_in_jet == 0 )); then
            echo "[OK] Run ${runnum_str}: JET and CALO segments match perfectly."
        else
            echo "[MISMATCH] Run ${runnum_str}: Missing ${missing_in_calo} segments in CALO; missing ${missing_in_jet} in JET."
        fi
    done
    echo "[INFO] Segment consistency check completed."
    echo "========================================"
}
...
compute_event_counts() {
    ...
    export actual_events_calo_qa actual_events_mbd actual_events_after_runtime actual_events_after_livetime \
           actual_events_after_badtower actual_events_after_magnet actual_events_after_createDst
    export percent_runs_calo_qa percent_runs_mbd percent_runs_runtime percent_runs_livetime \
           percent_runs_badtower percent_runs_magnet percent_runs_createDst
    export percent_actual_events_calo_qa percent_actual_events_mbd percent_actual_events_after_runtime \
           percent_actual_events_after_livetime percent_actual_events_after_badtower \
           percent_actual_events_after_magnet percent_events_after_createDst
}
